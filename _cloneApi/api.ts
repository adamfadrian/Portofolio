import lapak from '@/public/Lapak.png'
import dashboard from '@/public/dashboard.png'
import escapist from '@/public/escapist.png'
import movie from '@/public/movie.png'

export const data = [
    {
        id: 1,
        image: lapak,
        name: 'Lapak-UMKM',
        description: 'LapakUMKM is a platform that helps micro, small, and medium-sized enterprises (MSMEs) in Indonesia market their products online. ',
        url: 'https://lapakumkm.netlify.app/',
        github: 'https://github.com/ALTA-LapakUMKM-Group-2/LapakUMKM-FE',

    },
    {
        id: 2,
        image: escapist,
        name: 'Escapist',
        description: 'Alterra dashboard is a project for cloning the Altera dashboard website. ',
        url: 'https://escapist-app.netlify.app/',
        github: 'https://github.com/GP3-group1/ALTA-Airbnb-FE',
    },
    {
        id: 3,
        image: dashboard,
        name: 'Immersive Dashboard',
        description: 'Alterra dashboard is a project for cloning the Altera dashboard website. ',
        url: 'https://alta-dashboard-immersive.vercel.app/',
        github: 'https://github.com/GP2-Team3/ALTA-Dashboard',
    },
    {
        id: 4,
        image: movie,
        name: 'Movie App',
        description: 'Alterra dashboard is a project for cloning the Altera dashboard website. ',
        url: 'https://mymovie-app-sable.vercel.app',
        github: 'https://github.com/adamfadrian/mymovie-app',
    }
] 

export const skills = [
    {
        id: 1,
        title: "HTML"
    },
    {
        id: 2,
        title: "CSS"
    },
    {
        id: 3,
        title: "JavaScript"
    },
    {
        id: 4,
        title: "Typescript",
    },
    {
        id: 5,
        title: "ReactJS",
    },
    {
        id: 6,
        title: "NextJS"
    },
    {
        id: 7,
        title: "TailwindCSS",
    },
    {
        id: 8,
        title: "Bootstrap",
    },
    {
        id: 9,
        title: "Framer-Motion"
    },
    {
        id: 10,
        title: "DaisyUI"
    },
    {
        id: 11,
        title: "Git"
    },
    {
        id: 12,
        title: "GitHub"
    }
]